...
Validator.prototype.validate = function(file) {
	...
	try {
		...
		this._fail(MESSAGES.PATH_INVALID.message.replace('{a}', file));
	}

	if (!stat.isFile()) {
		this._fail(MESSAGES.PATH_ISNT_FILE.message.replace('{a}', file));
		return;
	}
...
Validator.prototype._loadFile = function() {
	this._data = fs.readFileSync(this._file, this._settings.encoding);
 ...
 * @private
 */
Validator.prototype._loadSettingsRCconfig = function() {
	var stat;
 ...
 * @private
 */
Validator.prototype._loadSettingsEditorconfig = function() {
	var config, key, stat;
...
Validator.prototype._validateEndOfLine = function() {
...
Validator.prototype._report = function(data, linenumber, payload) {
	...
	if (!this._invalid[this._file]) {
		this._invalid[this._file] = {};
	}
	file = this._invalid[this._file];
 ...
 */
Validator.prototype.getInvalidLines = function(file) {
	if (!this._invalid[file]) {
		return {};
	}

	return this._invalid[file];
};
