			...
			var
				...
				validate = function(match, offset, original) {
					...
					if (!self._ignoredLines[atLine + 1]) {
						...
						self._invalidate(
							merge({}, MESSAGES.NEWLINE_MAXIMUM, {
								line: atLine + 1,
								message: message
							}),
							{
...
Validator.prototype._validateNewlinesEOF = function() {
	if (this._settings.newline && this._lines.length > 1) {
		...
		if (this._lines[index].length > 0) {
			this._invalidate(merge({}, MESSAGES.NEWLINE, {line: index + 1}));
		}

		// check line before last line:
		if (this._lines[index - 1].length === 0) {
			this._invalidate(merge({}, MESSAGES.NEWLINE_AMOUNT, {line: index + 1}));
		}
	}
};
...
Validator.prototype._validateTrailingspaces = function(line, index) {
	if (this._settings.trailingspaces && typeof line === 'string') {
		...
		if (matchSpaces.length > 0 && matchSpaces[0].length > 0) {
			this._invalidate(merge({}, MESSAGES.TRAILINGSPACES, {line: index + 1}));
		}
	}
};
		...
		switch (this._settings.indentation) {
			case 'tabs':
				if (!tabsRegExp.test(line)) {
					// indentation failed...
					return this._invalidate(merge({}, MESSAGES.INDENTATION_TABS, {line: index + 1}));
				}
				break;
			...
			case 'spaces':
				if (!spacesRegExp.test(line)) {
					// Indentation failed...
					this._invalidate(merge({}, MESSAGES.INDENTATION_SPACES, {line: index + 1}));
				} else {
					...
					if (typeof this._settings.spaces === 'number') {
						...
						if (indent % this._settings.spaces !== 0) {
							...
							this._invalidate(
								merge({}, MESSAGES.INDENTATION_SPACES_AMOUNT, {
									line: index + 1,
									message: message
								}),
								{
...
/**
 * Add an invalid line
 * @param {Number} data Data containing linenumber, message and errocode
 * @param {String} payload Optional data for the validation error
 */
Validator.prototype._invalidate = function(data, payload) {
	var
