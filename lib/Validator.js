...
Validator.prototype._validateEndOfLine = function() {
	if (typeof this._settings.end_of_line === 'string') {
		var
			isEOL = function(ch) {
				return ch === '\r' || ch === '\n';
			},
			...
			payload
		;
		switch (this._settings.end_of_line.toUpperCase()) {
		case 'CR':
			desiredEOL = '\r';
			break;
		case 'LF':
		...
		for (pos = 0; pos < this._data.length; pos++) {
			...
			if (isEOL(aCharacter)) {
				if (aCharacter === '\r' && this._data.length > pos + 1 && isEOL(this._data[pos + 1])) {
				...
				if (totalEOL !== desiredEOL) {
					payload = {
						expected: this._settings.end_of_line,
