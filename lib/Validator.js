...
Validator.prototype.validate = function(path) {
	...
	if (stat.isFile()) {
		...
		this._lines.forEach(function(line, index) {
			...
			self._validateLinelength(line, index);
		});
...
Validator.prototype._validateLinelength = function(line, index) {
	if (this._settings.linelength && typeof line === 'string') {
		var
			payload,
			message,
			data,
		...
		// Is the line longer than defined?
		if (length > this._settings.linelength) {

			// Build message and report:
			message = MESSAGES.LINELENGTH.message
				.replace('{a}', length)
				.replace('{b}', this._settings.linelength);

			data = {message: message};
			data = merge({}, MESSAGES.LINELENGTH, data);
			payload = {
				length: length,
				maximum: this._settings.linelength
			};
