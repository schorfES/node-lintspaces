...
const tabsRegExp = /^\t*(?!\s).*$/; // leading tabs without leading spaces
const tabsRegExpForBOM = /^\t*(?! |\t).*$/; // leading tabs without leading spaces (allows BOM)
const tabsLeadingRegExp = /^(\t*).*$/; // leading tabs
const spacesRegExp = /^ *(?!\s).*$/; // leading spaces without leading tabs
const spacesRegExpForBOM = /^ *(?!\t).*$/; // leading spaces without leading tabs (allows BOM)
const spacesLeadingRegExp = /^( *).*$/; // leading spaces
...
class Validator {
	...
	_loadIgnores() {
		...
		if (Array.isArray(ignores)) {
			...
			ignores.forEach(function(expression) {
				...
				const matches = self._data.match(expression) || [];
	...
	_validateNewlineMaximum() {
		if (typeof this._settings.newlineMaximum === 'number') {
			if (this._settings.newlineMaximum > 0) {
				...
				const newlinesAtBeginn = '^(?:' + eol + '){' + (this._settings.newlineMaximum + 1) + ',}';
				...
				const newlinesInFile = '(?:' + eol + '){' + (this._settings.newlineMaximum + 2) + ',}';

				// Define function which is used as fake replace cycle to
				// validate matches:
				const validate = function (match, offset, original) {
					const substring = original.substr(0, offset);
					const newlines = substring.match(new RegExp(eol, 'g'));
					let amount = match.match(new RegExp(eol, 'g')).length;
					let atLine = 0;
					let message;
					let data;
					let line;
	...
	_validateIndentation(line, index) {
		if (!this._ignoredLines[index] &&
			...
			const tabsRegExpFinal = (this._settings.allowsBOM ? tabsRegExpForBOM : tabsRegExp);
			const spacesRegExpFinal = (this._settings.allowsBOM ? spacesRegExpForBOM : spacesRegExp);
			let spacesExpected;
			let indent;
			let message;
			let data;
	...
	_guessIndentation(line, index) {
		if (!this._ignoredLines[index] &&
			...
			const regExp = this._settings.indentation === 'tabs' ? tabsLeadingRegExp : spacesLeadingRegExp;
